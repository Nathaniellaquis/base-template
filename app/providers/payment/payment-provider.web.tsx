...
// Web-specific Stripe imports
import { Elements as StripeWebProvider } from '@stripe/react-stripe-js';
...
function PaymentProviderInner({ children }: { children: React.ReactNode }) {
  ...
  const subscribe = useCallback(async (targetPlan: PlanType, billingPeriod: BillingPeriod = 'monthly') => {
    ...
    try {
      ...
      trackFeature.use('subscription_started', {
        plan: targetPlan,
        period: billingPeriod,
        price,
        currency: 'USD',
        previousPlan: plan,
        action: subscriptionResponse.action,
      });
...
// Main PaymentProvider for Web - wraps with Stripe Elements
export function PaymentProvider({ children }: { children: React.ReactNode }) {
  if (!stripePromise) {
    console.error('Stripe publishable key not found for web');
    return <PaymentProviderInner>{children}</PaymentProviderInner>;
  }
